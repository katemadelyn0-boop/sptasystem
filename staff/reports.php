...
// Staff sees school fees only, admin sees all
$managed_filter = ($role==='staff') ? "AND pc.managed_by='staff'" : "";

$where="WHERE MONTH(p.payment_date)=? AND YEAR(p.payment_date)=? $managed_filter";
$p=[$month,$year];$t='ii';
if($sf){$where.=" AND p.status=?";$p[]=$sf;$t.='s';}

$s=$db->prepare("SELECT COUNT(*) c,COALESCE(SUM(p.amount_paid),0) total FROM payments p JOIN payment_requirements pr ON p.requirement_id=pr.requirement_id JOIN payment_categories pc ON pr.category_id=pc.category_id $where AND p.status='paid'");
...
$pageTitle = ($role==='staff') ? 'School Fee Reports' : 'Financial Reports';
?>
<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title><?=$pageTitle?> — SPTA System</title>
...
<div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
  <div><h1><?=$pageTitle?></h1><p><?=($role==='staff')?'School fee payment records by month and year.':'Payment records by month, year, and status.'?></p></div>
